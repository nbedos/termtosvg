...
class TerminalController:
    ...
    def render(self, template):
        ...
        '' (if it's not).
        """
        s = re.sub(r'\$\$|\${\w+}', self._render_sub, template)
        return s
    ...
    def _render_sub(self, match):
        ...
        if s == '$$':
            ...
            return getattr(self, s[2:-1]).decode('utf-8')
...
class ProgressBar:
    ...
    def update(self, percent, message):
        ...
        print(n)
        byte_line = (self.term.BOL + self.term.UP + self.term.CLEAR_EOL +
                     (self.bar % (100 * percent, '=' * n, '-' * (self.width - 10 - n))).encode('utf-8') +
                     self.term.CLEAR_EOL + message.center(self.width).encode('utf-8'))
        sys.stdout.write(byte_line.decode('utf-8'))
...
if __name__ == '__main__':
    ...
    p = ProgressBar(t, 'Progress bar')
    for i in range(10):
        time.sleep(1)
        p.update(0.1 * (i+1), "Reached {}%".format(10 * (i + 1)))
